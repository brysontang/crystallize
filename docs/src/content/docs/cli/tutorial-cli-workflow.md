...
- Tabs – `Logs`, `Summary`, `LLM Data` (machine-readable `<CrystallizeSummary>` XML for prompts/agents), `Errors`. `t` flips between rich and plain-text output, `S` focuses the summary tab.
...
- **LLM Data** – copyable `<CrystallizeSummary>` XML reflecting the same metrics/hypotheses/artifact links, ideal for pasting into an LLM for analysis.
